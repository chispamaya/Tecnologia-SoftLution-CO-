...
if ($tipoLike) {
    if ($tipoLike['likeOdislike'] == 1) {
        $borra = "DELETE FROM valorar WHERE nombreUsuario = '" . $nombre . "' AND idPost = '" . $idP . "'";
        $borrar = $link->query($borra);
        echo"D";
    } else {
        $actualiza = "UPDATE valorar SET likeOdislike = '" . 1 . "' WHERE nombreUsuario = '" . $nombre . "' AND idPost = '" . $idP . "'";
        $actualizar = $link->query($actualiza);
        echo"U";
    }
} else {
    $crea = "INSERT INTO valorar(likeOdislike, nombreUsuario, idPost) VALUES('" . 1 . "', '" . $nombre . "', '" . $idP . "')";
